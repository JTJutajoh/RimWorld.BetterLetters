    ...
    public static IEnumerable<CodeInstruction> DoArchivableRow(IEnumerable<CodeInstruction> instructions)
    {
        ...
        for (int i = 0; i < codes.Count; i++)
        {
            ...
            if (codes[i]!.opcode == OpCodes.Stloc_S &&
                (((LocalBuilder)codes[i]!.operand!)).LocalIndex == 4) //IL_0124 in 1.6
            {
                // Store the label for the end of the if block we want to skip
                var labelOfEndOfIfBlock = codes[i + 3]!.operand as Label?;
                ...
                {
                    if (labelOfEndOfIfBlock is not null &&
                        (codes[i2]!.labels?.Contains(labelOfEndOfIfBlock.Value) ?? false))
                    {
                        // Store the label for the end of the else block we want to skip
                        labelOfEndOfElseBlock = codes[i2 - 1]!.operand as Label?;
                        break;
                    }
            ...
            if (codes[i]!.opcode == OpCodes.Ldstr && (string)codes[i]!.operand! == "PinArchivableTip") //IL_02AC in 1.6
            {
                ...
                yield return CodeInstruction.CallClosure<Action<Rect, IArchivable, string, int>>((rect, archivable, key,
                    arg) =>
                {
            ...
            if (i > 4 && !hasPatchedClickBehavior && codes[i - 1]!.opcode == OpCodes.Brfalse_S &&
                codes[i - 2]!.Calls(ButtonPatchMethodAnchor)) //IL_02D1 in 1.6
            {
                // Load the archivable (the letter) onto the stack
                yield return new CodeInstruction(OpCodes.Ldarg_2);
                ...
                {
                    if (codes[j]!.opcode != OpCodes.Br_S || codes[j]!.operand is not Label label) continue;
